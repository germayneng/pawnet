...
Most of the source codes of this project, `pawnet` are stored in the `pawnet` directory.
Generated Docker container mounts the project directory to ``./project`` of the container's `dockeruser`'s home directory and therefore
...
`make create-container` creates and launch the pawnet container.
...
To launch Jupyter Notebook, please run `make jupyter` on host (after `make create-container`). After launching the Jupyter Notebook, you can
